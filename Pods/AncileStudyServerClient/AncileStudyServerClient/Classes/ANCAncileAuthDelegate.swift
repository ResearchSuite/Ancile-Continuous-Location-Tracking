...
open class ANCAncileAuthDelegate: NSObject, RSRedirectStepDelegate {
    
    open static func getQueryStringParameter(url: String, param: String) -> String? {
        guard let url = URLComponents(string: url) else { return nil }
        
        return url.queryItems?.first(where: { $0.name == param })?.value
    }
    
    open static func safeOpenURL(url: URL) {
    ...
    fileprivate weak var client: ANCClient!
    fileprivate var urlScheme: String
    fileprivate var authCompletion: ((Error?) -> ())? = nil
    ...
    open func redirectViewControllerDidLoad(viewController: RSRedirectStepViewController) {
        
    }
    
    open func beginRedirect(completion: @escaping ((Error?) -> ())) {
